            ...
            .then(result => {
                if (result.success) {
                    const filtered = (result.data || []).filter((w: WorkItem) => 
                        ['videography', 'secondary-videography'].includes(w.task_key || '')
                    );
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {works.map(w => (
                                <tr key={w.lead_employee_id} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-4 py-3 text-gray-600 flex items-center gap-1"><Video size={13} className="text-green-500" /> {w.task_name || w.name}</td>
