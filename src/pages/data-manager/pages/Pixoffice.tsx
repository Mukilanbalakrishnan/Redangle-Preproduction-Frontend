...
export default function Pixoffice() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/pixoffice/submit`, data);
            if (response.data.success) {
                setSubmitted(true);
            } else {
                alert("Failed to save Pixoffice data: " + response.data.message);
            }
        } catch (err: any) {
            console.error("Error submitting pixoffice data", err);
    ...
    if (submitted) {
        return (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <h2 className="text-lg font-bold mb-1" style={{ color: '#111827' }}>Pixoffice Data Submitted!</h2>
    ...
    if (view === 'list') {
        return (
            <div>
                <div className="mb-6">
                    <h1 className="text-lg font-bold" style={{ color: '#111827' }}>{stageScope.label} Pixoffice</h1>
                    <p className="text-sm" style={{ color: '#6B7280' }}>Select a {stageScope.label.toLowerCase()} lead to process for Pixoffice</p>
                </div>
    ...
    return (
        <div>
            <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h1 className="text-lg font-bold" style={{ color: '#111827' }}>Pixoffice Management</h1>
                    <p className="text-sm" style={{ color: '#6B7280' }}>Manage and trigger QC for Pixoffice data entries</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm mb-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label className="block text-xs font-semibold mb-2" style={{ color: '#374151' }}>Pixoffice Storage Path</label>
                        <input
                            type="text"
                            placeholder="e.g. \\\\NAS-01\\Pixoffice\\SharmaWedding"
                            value={storagePath}
            ...
            >
                <button
                    ...
                    style={{ background: 'linear-gradient(135deg, #a78bfa, #5B5FC7)' }}
                >
                    <Upload size={16} /> {isSubmitting ? 'Submitting...' : 'Submit — Set as QC Pending'}
                </button>
            </div>
        </div>
    )
