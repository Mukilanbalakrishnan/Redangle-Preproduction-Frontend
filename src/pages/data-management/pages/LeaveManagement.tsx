...
export default function LeaveManagement() {
    ...
    const fetchLeaves = async () => {
        ...
        try {
            ...
            const res = await axios.get(`${API_URL}/leave?role=data-management`)
            if (res.data?.success) {
    ...
    const handleDownloadReport = () => {
        ...
        const headers = ['Employee', 'Role', 'Type', 'Dates', 'Days', 'Status', 'Reason'];
        const csvRows = filteredLeaves.map(l => [
            l.emp, l.role, l.type, l.dates, l.no_of_days, l.status, l.reason
        ...
        link.setAttribute("download", `leave_management_${dateStr}.csv`);
            ...
            <div className="flex items-start justify-between mb-5">
                ...
                <div className="flex items-center gap-3">
                    {[
                        ...
                        <div key={s.label} className="flex items-center gap-2 px-4 py-2.5 bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <table className="w-full">
                    <thead>
                        <tr style={{ background: '#FAFAFA', borderBottom: '1px solid #E5E7EB' }}>
                            {['Employee', 'Role', 'Leave Type', 'Dates', 'Days', 'Status', 'Actions'].map(h => (
                    ...
                    <tbody>
                        {loading ? (
                            <tr>
                                <td colSpan={7} className="text-center px-5 py-12 text-sm text-gray-500">Loading requests...</td>
                            </tr>
                            ...
                            <tr key={i} style={{ borderTop: '1px solid #F3F4F6' }} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-4 py-4 text-sm font-semibold" style={{ color: '#374151' }}>{l.no_of_days}</td>
                                <td className="px-4 py-4">
                                    <span style={{
                                        display: 'inline-flex', alignItems: 'center', padding: '3px 12px', borderRadius: '9999px',
