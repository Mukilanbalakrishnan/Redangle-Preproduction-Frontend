...
export default function MyAttendance() {
    ...
    const fetchTodayAttendance = async () => {
        if (!adminId || String(adminId).includes('NaN')) return
        try {
            const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'
            const res = await axios.get(`${API_URL}/employee/${adminId}/attendance/today`)
    ...
    const fetchLeaves = async () => {
        ...
        try {
            const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'
            const res = await axios.get(`${API_URL}/employee/${adminId}/leave-requests`)
            if (res.data?.success) {
    ...
    useEffect(() => {
        if (adminId && !String(adminId).includes('NaN')) {
    ...
    const handlePunchIn = async () => {
        ...
        try {
            const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'
            await axios.post(`${API_URL}/employee/${adminId}/punch-in`)
    ...
    const handlePunchOut = async () => {
        ...
        try {
            const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'
            await axios.post(`${API_URL}/employee/${adminId}/punch-out`)
    ...
    const handleLeaveSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5001/api'
            await axios.post(`${API_URL}/employee/${adminId}/leave-requests`, {
