...
export default function CandidVideo() {
    ...
    const fetchProjects = () => {
        ...
        getEmployeeProjectsByType(employeeId, 'Candid')
            .then(res => {
                const filtered = (res.data.data || []).filter((p: AssignedProject) => p.status.toLowerCase() !== 'pending')
            ...
            .catch(err => console.error('CandidVideo projects fetch error', err))
    ...
    return (
        <div className="space-y-6 max-w-7xl pb-20 animate-in zoom-in-95 duration-300">
            {!selectedProject ? (
                <>
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 mb-1 font-sans">Candid Video Workspace</h1>
                <p className="text-sm text-gray-500 font-medium pb-2">Manage and design your accepted Candid Video projects</p>
            </div>
                                ...
                                <tr>
                                    <td colSpan={5} className="px-6 py-12 text-center">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-sm font-bold text-gray-600">No accepted projects found</p>
                                            <p className="text-xs text-gray-400 mt-1">Accept projects from the 'Assigned Projects' page first</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                            ...
                            <div>
                                ...
                                <div className="bg-purple-50/50 p-4 rounded-2xl mb-6">
                                    <p className="text-[13px] text-gray-700 font-medium leading-relaxed">
                                        {selectedProject.admin_notes || "Create a cinematic 'Candid Video' deliverable. Follow standard practices for candid highlight flows. Emphasize emotional moments and natural lighting."}
                                    </p>
                                </div>
                            </div>
