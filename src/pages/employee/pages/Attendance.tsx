...
import { Calendar, LogIn, LogOut, Clock, ChevronDown, CheckCircle2, Users } from 'lucide-react'
...
export default function Attendance() {
    ...
    const [today, setToday] = useState<{ check_in: string | null; check_out: string | null }>({ check_in: null, check_out: null })
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            if (res.data?.success && res.data.data) {
                ...
                let statsData = { totalDays: 0, present: 0, absent: 0, percentage: 0 }
                ...
                if (Array.isArray(res.data.data)) {
                    ...
                    if (res.data.data.stats) {
                        statsData = res.data.data.stats
                    }
                }
                
                setHistory(records)
                const total = statsData.totalDays || records.length
                const present = statsData.present || records.filter(r => r.status?.toLowerCase() === 'present').length
                const absent = statsData.absent || records.filter(r => r.status?.toLowerCase() === 'absent').length
                const percentage = statsData.percentage || (total > 0 ? Math.round((present / total) * 100) : 0)
    ...
    return (
        <div className="space-y-6 max-w-6xl">
            ...
            <div>
                ...
                <p className="text-sm text-gray-500 font-medium">Punch in/out and track your attendance records</p>
            </div>

            {/* Punch In/Out Card */}
            <div className="bg-white p-6 rounded-[24px] border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { label: 'Total Working Days', value: stats.total,      color: 'text-green-500',  bg: 'bg-green-100',  icon: <Users size={20} /> },
                    { label: 'Present',            value: stats.present,    color: 'text-red-500',    bg: 'bg-red-100',    icon: <Users size={20} /> },
                    { label: 'Absent',             value: stats.absent,     color: 'text-orange-400', bg: 'bg-orange-100', icon: <Users size={20} /> },
                    { label: 'Attendance %',       value: `${stats.percentage}%`, color: 'text-blue-500', bg: 'bg-blue-100', icon: <Users size={20} /> },
                ].map(card => (
                    <div key={card.label} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-xs font-bold tracking-wide text-gray-500 mb-2">{card.label}</p>
                            <h3 className={`text-2xl font-bold ${card.color}`}>{card.value}</h3>
                        </div>
                        <div className={`w-10 h-10 ${card.bg} rounded-xl flex items-center justify-center ${card.color} shrink-0`}>
                            {card.icon}
                        </div>
                    </div>
                ))}
                ...
                <div className="p-4">
                    <table className="w-full">
                        ...
                        <tbody>
                            {history.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="px-6 py-10 text-center text-sm text-gray-400">No attendance records found.</td>
                                </tr>
                            ) : (
                                history.map((record, i) => {
                                    const loginTime  = formatTime(record.check_in)
                                    const logoutTime = formatTime(record.check_out)
                                    const statusStyle = record.status?.toLowerCase() === 'present'
                                    ...
                                    return (
                                        <tr key={i} className="text-center hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-5 text-sm font-bold text-gray-700">
                                                <div className="flex items-center justify-center gap-2">
                                                    <LogIn size={16} className={loginTime !== '--' ? 'text-green-500' : 'text-gray-400'} />
                                                    {loginTime}
                                                </div>
                                            </td>
                                            <td className="px-6 py-5 text-sm font-bold text-gray-700">
                                                <div className="flex items-center justify-center gap-2">
                                                    <LogOut size={16} className={logoutTime !== '--' ? 'text-red-500' : 'text-gray-400'} />
                                                    {logoutTime}
                                                </div>
                                            </td>
                                            <td className="px-6 py-5">
                                                <span className={`px-5 py-1.5 rounded-full text-xs font-semibold ${statusStyle}`}>
